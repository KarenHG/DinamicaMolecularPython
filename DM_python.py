...
def fuerza(rx,ry,rz):
    ...
    for i in numba.prange(1,nat):
        for j in range(i+1,nat+1):
            ...
            dx = dx-np.rint(dx*boxix)*boxx
            dy = dy-np.rint(dy*boxiy)*boxy
            dz = dz-np.rint(dz*boxiz)*boxz
...
def mover(istepp):
    ...
    rx = rx - np.rint(rx*boxix)*boxx
    ry = ry - np.rint(ry*boxiy)*boxy
    rz = rz - np.rint(rz*boxiz)*boxz
